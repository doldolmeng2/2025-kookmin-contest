...
#
# 구독:
#   rubbercone_info   (std_msgs/Int32MultiArray)  - [오프셋, end_flag]
...
class MainNode(Node):
    ...
    def rubbercone_callback(self, msg: Int32MultiArray):
        """라바콘 노드에서 발행한 조향 오프셋과 종료 플래그를 수신한다."""
        if len(msg.data) >= 2:
    ...
    def control_cycle(self):
        ...
        # 속도 급변 방지: 목표 속도까지 0.1씩 점진적으로 증가
        ...
        # ── 디버그 상태 창 ───────────────────────────────────────────────────
        # OpenCV 창에 현재 상태 텍스트를 렌더링하여 실시간 모니터링에 활용한다.
